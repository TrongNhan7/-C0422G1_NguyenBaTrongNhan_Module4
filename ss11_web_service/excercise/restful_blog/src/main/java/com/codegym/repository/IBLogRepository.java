...
public interface IBLogRepository extends JpaRepository<Blog, Integer> {
    ...
    @Query(value = "select  * from Blog where status=0 and blog.id_category = :id ", nativeQuery = true,
            countQuery = "select count(*) from (select  * from Blog where status=0 and blog.id_category = :id) table_name ")
    Page<Blog> findByIdCategory(@Param("id") Integer id, Pageable pageable);
